...
import {API_DELETE_DISCOUNT} from "@/config/api";
...
const TableDiscount = () => {
    ...
    const handleClickDeleteDiscount = (discountToDeleted: Discount) => {
        setDiscount(discountToDeleted);
        setIsOpenDeleteModal(true);
    }

    const handleDelete = async () => {
        await deleteData (API_DELETE_DISCOUNT + '/' + discount?.id)
                        ...
                        {discounts.map((discount: Discount, key: number) => (
                            <tr key={key} className="text-xs">
                                ...
                                <td className="border-b border-[#eee] px-2 py-3 dark:border-strokedark">
                                    ...
                                    >
                                        {
                                            discount.status === "ACTIVE" ? "Đang Hoạt động" : "Tạm ngưng"
                                        }
                                    </p>
                                </td>
                                <td className="border-b border-[#eee] px-2 py-3 dark:border-strokedark">
