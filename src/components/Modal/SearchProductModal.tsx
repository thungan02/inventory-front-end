...
import {ProductCart, ProductSearch} from "@/models/Product";
...
const columns: string[] = ["Sản phẩm", "Quy cách đóng gói", "Trọng lượng", "Sẵn có", "Giá (đ)"];

const SearchProductModal = ({onClose, products, setProducts}: SearchProductModalProps) => {
    ...
    const handleSearchProductByProductName = async () => {
        if (productName !== '') {
            const data: ProductSearch[] = await getData(API_GET_ALL_PRODUCTS + "?name=" + productName);
            const filteredData = data.filter(searchProduct => !products.some(product => product.id === searchProduct.id));
